...
const createViewModel = (name) => {
    return new Promise((resolve, reject) => {
        ...
        const vmName = name.replace(/-([a-z])/g, function (g) { return g[1].toUpperCase(); });

        let viewModelTemplate =
            `define(['knockout'],
    function (ko) {
        function ${vmName}ViewModel (context) {
            let self = this;
        
        }
        return ${vmName}ViewModel;
    }
);`;
...
exports.init = () => {
    ...
    exports.checkForRootFolder()
        ...
        .then((res)=>{
            return saveFiles(res.name, res.description, res.version, res.styleOpt);
        })
        .then(()=>{
            logger(colors.blue('Done...!'), "both");
        })
        .catch((err)=>{
            return logger(colors.red(err), "both");
        })
};
